function compileHTML(field,e){
    document.getElementById("output").innerHTML=field.value;
}
function f1(){
    document.getElementById("div1").style.display="block";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f2(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="block";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f3(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="block";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f4(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="block";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f5(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="block";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f6(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="block";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f7(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="block";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f8(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="block";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f9(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="block";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f10(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="block";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f11(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="block";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f12(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="block";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f13(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="block";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f14(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="block";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f15(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="block";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";    
}
function f16(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="block";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f17(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="block";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f18(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="block";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f19(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="block";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f20(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="block";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f21(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="block";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f22(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="block";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f23(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="block";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f24(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="block";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f25(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="block";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f26(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="block";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f27(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="block";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f28(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="block";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f29(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="block";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f30(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="block";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="none";
}
function f31(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="block";
    document.getElementById("div32").style.display="none";
}
function f32(){
    document.getElementById("div1").style.display="none";
    document.getElementById("div2").style.display="none";
    document.getElementById("div3").style.display="none";
    document.getElementById("div4").style.display="none";
    document.getElementById("div5").style.display="none";
    document.getElementById("div6").style.display="none";
    document.getElementById("div7").style.display="none";
    document.getElementById("div8").style.display="none";
    document.getElementById("div9").style.display="none";
    document.getElementById("div10").style.display="none";
    document.getElementById("div11").style.display="none";
    document.getElementById("div12").style.display="none";
    document.getElementById("div13").style.display="none";
    document.getElementById("div14").style.display="none";
    document.getElementById("div15").style.display="none";
    document.getElementById("div16").style.display="none";
    document.getElementById("div17").style.display="none";
    document.getElementById("div18").style.display="none";
    document.getElementById("div19").style.display="none";
    document.getElementById("div20").style.display="none";
    document.getElementById("div21").style.display="none";
    document.getElementById("div22").style.display="none";
    document.getElementById("div23").style.display="none";
    document.getElementById("div24").style.display="none";
    document.getElementById("div25").style.display="none";
    document.getElementById("div26").style.display="none";
    document.getElementById("div27").style.display="none";
    document.getElementById("div28").style.display="none";
    document.getElementById("div29").style.display="none";
    document.getElementById("div30").style.display="none";
    document.getElementById("div31").style.display="none";
    document.getElementById("div32").style.display="block";
}



function op1(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>My first HTML Page.</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op2(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h2>The href Attribute</h2>\n"+
    "<a href='http://www.vlab.co.in/'>Visit Vlab</a>\n"+
    "</body>\n"+
    "</html>\n";
}
function op3(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>Button Tag</p>\n"+
    "<button type='button'>Click Me!</button>\n"+
    "</body>\n"+
    "</html>\n";
}
function op4(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p><center>This is a Center Tag</center></p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op5(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<details>\n"+
    "<summary>Click to see details</summary>\n"+
    "<p>You are working on a Detail tag.</p>\n"+
    "</details>\n"+
    "</body>\n"+
    "</html>\n";
}
function op6(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h1>The footer element</h1>\n"+
    "<footer>\n"+
      "<p>This will be shown on bottom <br>\n"+
      "<a href='website:123@example.com'>123@example.com</a></p>\n"+
    "</footer>\n"+
    "</body>\n"+
    "</html>\n";
}
function op7(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<form action='/action_page.php' method='get'>\n"+
    "<label for='fname'>First name:</label>\n"+
    "<input type='text' id='fname' name='fname'><br>\n"+
    "<label for='lname'>Last name:</label>\n"+
    '<input type=text" id="lname" name="lname"><br><br>\n'+
    '<input type="submit" value="Submit">\n'+
    "</form>\n"+
    "</body>\n"+
    "</html>\n";
}
function op8(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h1>The iframe element</h1>\n"+
    '<iframe src="http://www.vlab.co.in/" title="Virtual Lab">\n'+
    "</iframe>\n"+
    "</body>\n"+
    "</html>\n";
}
function op9(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h1>Heading 1</h1>\n"+
    "<h2>Heading 2</h2>\n"+
    "<h3>Heading 3</h3>\n"+
    "<h4>Heading 4</h4>\n"+
    "<h5>Heading 5</h5>\n"+
    "<h6>Heading 6</h6>\n"+
    "</body>\n"+
    "</html>\n";
}
function op10(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h2>Text Area</h2>\n"+
    '<textarea id="" name="" rows="4" cols="50">This is a text area we can input here and can increase or decrease the size.</textarea>\n'+
    "</body>\n"+
    "</html>\n";
}
function op11(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    '<img src="images/123.jpg" alt="Spiderman" height="150px" width="200px">\n'+
    "</body>\n"+
    "</html>\n";
}
function op12(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>Enter Value</p>\n"+
    '<input type="text" id="fname" name="fname">\n'+
    '<input type="submit" value="Submit">\n'+
    "</body>\n"+
    "</html>\n";
}
function op13(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<i>This text is italic</i>\n"+
    "<b>This text is bold</b>\n"+
    "<strong>This text is important!</strong>\n"+
    "<em>This text is emphasized</em>\n"+
    "<small>This is some smaller text.</small>\n"+
    "<p>Do not forget to buy <mark>milk</mark> today.</p>\n"+
    "<p>My favorite color is <del>blue</del> red.</p>\n"+
    "<p>My favorite color is <del>blue</del> <ins>red</ins>.</p>\n"+
    "<p>This is <sub>subscripted</sub> text.</p>\n"+
    "<p>This is <sup>superscripted</sup> text.</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op14(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<form action='/action_page.php' method='get'>\n"+
    "<label for='fname'>First name:</label>\n"+
    "<input type='text' id='fname' name='fname'><br>\n"+
    "<label for='lname'>Last name:</label>\n"+
    '<input type=text" id="lname" name="lname"><br><br>\n'+
    '<input type="submit" value="Submit">\n'+
    "</form>\n"+
    "</body>\n"+
    "</html>\n";
}
function op15(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<a href='http://www.vlab.co.in/' target='_blank'>Visit Virtual lab website!</a>\n"+
    "</body>\n"+
    "</html>\n";
}
function op16(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h2>An Unordered HTML List</h2>\n"+

    "<ul>\n"+
      "<li>Coffee</li>\n"+
      "<li>Tea</li>\n"+
      "<li>Milk</li>\n"+
    "</ul>\n"+  
    
    "<h2>An Ordered HTML List</h2>\n"+
    
    "<ol>\n"+
      "<li>Coffee</li>\n"+
      "<li>Tea</li>\n"+
      "<li>Milk</li>\n"+
    "</ol>\n"+
    "</body>\n"+
    "</html>\n";
}
function op17(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p><h1>The nav element</h1>\n"+
    '<nav>\n'+
    '<a href="/html/">HTML</a> |\n'+
    '<a href="/css/">CSS</a> |\n'+
    '<a href="/js/">JavaScript</a> |\n'+
    '<a href="/python/">Python</a>\n'+
    '</nav>\n'+
    "</body>\n"+
    "</html>\n";
}
function op18(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>Scroll UP</p>\n"+
    '<marquee width="60%" direction="up" height="100px">\n'+
    'This is a sample scrolling text that has scrolls in the upper direction.\n'+
    '</marquee>\n'+
    "</body>\n"+
    "</html>\n";
}
function op19(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<head>\n"+
    "<meta name='keywords' content='HTML,CSS,XML,JavaScript'>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>All meta information goes in the head section...</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op20(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<label for='Bike'>Choose a bike:</label>\n"+

    '<select id="bike">\n'+
      '<option value="Hero">Hero</option>\n'+
      '<option value="Honda">Honda</option>\n'+
      '<option value="Tvs">Tvs</option>\n'+
      '<option value="BMW">BMW</option>\n'+
    "</select>\n"+
    "</body>\n"+
    "</html>\n";
}
function op21(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    '<form oninput="x.value=parseInt(a.value)+parseInt(b.value)"><input type="range" id="a" value="50">+<input type="number" id="b" value="25">=<output name="x" for="a b"></output>\n'+
    '</form>\n'+
    "</body>\n"+
    "</html>\n";
}
function op22(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>My first Paragraph.</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op23(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p><u>My first HTML Page</u></p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op24(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>Pre tag eElement.</p>\n"+
    '<pre>\n'+
    'Text in a pre element\n'+
    'is displayed in a fixed-width\n'+
    'font, and it preserves\n'+
    'both      spaces and\n'+
    'line breaks\n'+
    '</pre>\n'+
    "</body>\n"+
    "</html>\n";
}
function op25(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<p>blockquote</p>\n"+
    "<blockquote cite='http://www.worldwildlife.org/who/index.html'> India, country that occupies the greater part of South Asia</blockquote>\n"+
    "<p><q>Abbrevations</q></p>\n"+
    "<p>The <abbr title='World Health Organization'>WHO</abbr> was founded in 1948.</p>\n"+
    "<p>Cite</p>\n"+
    "<p>Painting was made by<cite>Jackson Pollock</cite></p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op26(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    '<dialog open>This is an open dialog window</dialog>\n'+
    "</body>\n"+
    "</html>\n";
}
function op27(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    '<h2>1st Section</h2>\n'+
    "<p>First Section of the page</p>\n"+
    '<h2>2nd Section</h2>\n'+
    "<p>Second Section of the page</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op28(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<label for='Bike'>Choose a bike:</label>\n"+
    '<select id="bike">\n'+
    '<option value="Select">Select</option>\n'+
      '<option value="Hero">Hero</option>\n'+
      '<option value="Honda">Honda</option>\n'+
      '<option value="Tvs">Tvs</option>\n'+
      '<option value="BMW">BMW</option>\n'+
    "</select>\n"+
    "</body>\n"+
    "</html>\n";
}
function op29(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h1>The span element</h1>\n"+
    '<p>My mother has <span style="color:blue;font-weight:bold">blue</span> eyes and my father has <span style="color:darkolivegreen;font-weight:bold">dark green</span> eyes.</p>\n'+
    "</body>\n"+
    "</html>\n";
}
function op30(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    '<style>\n'+
    '#p1{background-color:yellow;}\n'+
    '</style>\n'+
    "</head>\n"+
    "<body>\n"+
    "<p id='p1'>My first HTML Page.</p>\n"+
    "</body>\n"+
    "</html>\n";
}
function op31(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    "<h2>Basic HTML Table</h2>\n"+
    "<table style='width:100%'>\n"+
        "<tr>\n"+
            "<th>Firstname</th>\n"+
            "<th>Lastname</th>\n"+ 
            "<th>Age</th>\n"+
        "</tr>\n"+
        "<tr>\n"+
            "<td>Jill</td>\n"+
            "<td>Smith</td>\n"+
            "<td>50</td>\n"+
        "</tr>\n"+
        "<tr>\n"+
            "<td>Eve</td>\n"+
            "<td>Jackson</td>\n"+ 
            "<td>94</td>\n"+
        "</tr>\n"+
        "<tr>\n"+
            "<td>John</td>\n"+
            "<td>Doe</td>\n"+ 
            "<td>80</td>\n"+
        "</tr>\n"+
    "</table>\n"+
    "</body>\n"+
    "</html>\n";
}
function op32(){
    var x = document.getElementById('textarea'); 
    x.value = "<!DOCTYPE html>\n"+
    "<html>\n"+
    "<head>\n"+
    "</head>\n"+
    "<body>\n"+
    '<video width="220" height="165" controls>\n'+
    '<source src="images/video.mp4" type="video/mp4">\n'+
    "</video>\n"+
    "</body>\n"+
    "</html>\n";
}